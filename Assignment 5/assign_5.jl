...
begin
	train_x, train_y = CIFAR10(split=:train)[:] #loadx is image data, y is label
	train_x = reshape(train_x, 32, 32, 3, :) #turn x array images to 3d array
	labels2 = onehotbatch(train_y, 0:9) #convert training labels to one hot encoded 
	classes = ["airplane", "automobile", "bird", "cat", "deer", "dog", "frog", "horse", "ship", "truck"] #label array
end;
...
image(x) = colorview(RGB, permutedims(x, (3, 2, 1)))
	...
	rand_ids = rand(1:size(train_x, 4)) # Select a random index
	rand_image = train_x[:, :, :, rand_ids] # Extract the corresponding image
	plot(image(rand_image)) # Use plot function to display the image
end
...
# ╔═╡ e4338b7a-97c3-4c12-a764-b97aa06c080d
begin
	train_x_small = train_x[:,:,:,1:50000]  
	train_y_small = train_y[1:50000]
	#labels2 = onehotbatch(train_y_small, 0:9)
	
	train = ([(train_x_small[:,:,:,i], labels2[:,i]) for i in partition(1:49000, 1000)]) |> gpu

	valset = 49001:50000
	valX = train_x_small[:,:,:,valset] |> gpu #image data for validation set
	valY = labels2[:, valset] |> gpu #label data for validation set
end
...
#CNN work on batches in parralell, so 1000 images go through it simulatneosly 
#change first conv layer 1 to 3, since rgb no longer greyscale
#input 32*32*3
m3 = Chain(
  Conv((5,5), 3=>16, pad=(2,2), relu), #output remain 32*32, 16 channels now 16 not 3
  MaxPool((2,2)), #reduce spatial dimension by half, 16*16 now
  Conv((5,5), 16=>8, pad=(2,2), relu), #pad is 2 so spatial 16*16, channel is 8
  MaxPool((2,2)), #half spatial dimension, so 8*8
  #Now image dimension is 8*8*8 = 512
...
#First dense layer changed to take output of previous conv and pool layer
#First conv make is 16*16
#Second conv make 8*8
#La
...
#m = 0.0001 - accuracy went down from 0.6950 to 0.6935
...
#compare predicted label to true label, onecold convert model output to class label
...
begin
	...
	epochs = 11
	for epoch = 1:epochs
		#loop through each batch, so 59 iterations
		#each batch, compute loss
  		for d in train
			#gradient of loss function with respect to parms
			#'do' gradient() uses, so gradient for parms with ('do') loss
    		gs = gradient(Flux.params(m3)) do 
      			l = loss2(d...) #loss function of current batch 'd', parallelized
				#line above put batch through CNN, then compute loss
    		end
...
begin
	#loaf cifar10 test set
	test_x, test_y = CIFAR10(split=:test)[:] #load x is image data, y is label
	test_x = reshape(test_x, 32, 32, 3, :) #turn x array images to 3d array
end;

# ╔═╡ ac32ec6d-9690-4e23-846c-5f9b98d934a1
test_labels = onehotbatch(test_y, 0:9); #one hot encode the labels

# ╔═╡ a15c5d9e-51eb-42e2-aea3-bf503dd0ff7d
md"""
Created test sets, for x and y. The code snipped below just displays the images
"""

# ╔═╡ d55df98c-a774-47de-a595-9f7b6631adaf
begin
	rnd_test = rand(1:size(test_x)[4]) #pick random indexes from total data set
	plot(image(test_x[:,:,:,rnd_test]), axis=false) #plot image, no axis
	annotate!(-1.0, 1.0, text(classes[test_y[rnd_test]+1], :blue, :right, 12)) #annotate
end
...
# ╔═╡ d2261dfd-fa14-46c0-9585-981d88409630
begin
	ids = rand(1:size(test_x)[4], 20) #20 random ids
	rand_test = test_x[:,:,:,ids] #get the images for the ids
	rand_truth = test_y[ids] #true labels for selected ids
	rand_out = m3(rand_test) #input random test images into neureal network
end

# ╔═╡ 742abbf5-3f9f-4cc5-8aa6-a00ea83571e5
@bind inx Slider(1:1:20, default=1)

# ╔═╡ 19ea31cf-ce9e-4d9a-afc4-2a3df6cbaff8
begin
	displayed_image = rand_test[:,:,:,inx] #get image at index of slider value
	displayed_label_index = argmax(rand_out[:, inx]) # Get the index of the predicted label
	predicted_label = classes[displayed_label_index]
	
	
	plot(image(displayed_image)) # Display image  
	title!("Predicted Label: $predicted_label") #Display predicted label
end
...
begin
	...
	test = [(test_x[:,:,:,i], test_labels[:,i]) for i in partition(1:size(test_x, 4), 1000)]
	accuracy(test[1]...)
end
...
begin
	...
	for i in 1:10
  		preds = m3(test[i][1]) #makes prediction on first elem of the ith batch
  		lab = test[i][2] #Retrieves the true labels for the images in the batch
  		for j = 1:1000 #for each element in current batch
    		pred_class = findmax(preds[:, j])[2] #most predicted class for image
    		actual_class = findmax(lab[:, j])[2] #actual class for image
    		if pred_class == actual_class
      			class_correct[pred_class] += 1 #correct prediction increment counter
    		end
    		class_total[actual_class] += 1 #total count
  		end
	end	
end
...
julia_version = "1.9.4"
manifest_format = "2.0"
project_hash = "a67b259ebfed91d6c5e65d2eff93f0ac9e125c2b"
